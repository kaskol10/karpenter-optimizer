...
function ComparisonView({ recommendations }) {
  ...
  const totalCurrentCost = recommendations.reduce(
    (sum, rec) => sum + (rec.currentState?.estimatedCost || 0),
    0
  );
  ...
  const costSavingsPercent =
    totalCurrentCost > 0 ? ((costSavings / totalCurrentCost) * 100).toFixed(1) : 0;
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: '1.75rem', fontWeight: 600, color: '#333' }}>
            ...
            <span style={{ fontSize: '1rem', fontWeight: 400, color: '#666' }}>/hr</span>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ fontSize: '1.75rem', fontWeight: 600, color: '#10b981' }}>
            ${totalRecommendedCost.toFixed(2)}
            <span style={{ fontSize: '1rem', fontWeight: 400, color: '#666' }}>/hr</span>
          </div>
        </div>
        ...
        >
          ...
          <div
            style={{
              fontSize: '1.75rem',
              fontWeight: 600,
              color: costSavings > 0 ? '#10b981' : '#ef4444',
            }}
          >
            ${Math.abs(costSavings).toFixed(2)}
            <span style={{ fontSize: '1rem', fontWeight: 400, color: '#666' }}>/hr</span>
          </div>
